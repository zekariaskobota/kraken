...
const BottomNavigation = () => {
  ...
  return (
    <div className="md:hidden flex justify-around items-center bg-gradient-to-r from-[#0b0e14] to-[#1a1d29] py-3 pb-[calc(12px+env(safe-area-inset-bottom))] fixed bottom-0 left-0 right-0 w-full shadow-[0_-4px_12px_rgba(0,0,0,0.3)] border-t border-[rgba(42,45,58,0.5)] backdrop-blur-md z-[1000]">
      ...
      >
        <FaMoneyBillWave className={`text-xl transition-all duration-300 ${location.pathname === "/demo" ? "scale-110" : ""}`} />
        <span className="text-[11px] font-medium whitespace-nowrap overflow-hidden text-ellipsis max-w-full">Demo</span>
        {location.pathname === "/demo" && (
          <span className="absolute top-0 left-1/2 transform -translate-x-1/2 w-10 h-0.5 bg-gradient-to-r from-teal-400 to-cyan-400 rounded-b"></span>
        )}
      </Link>
      ...
      >
        <FaUser className={`text-xl transition-all duration-300 ${location.pathname === "/profile" ? "scale-110" : ""}`} />
        <span className="text-[11px] font-medium whitespace-nowrap overflow-hidden text-ellipsis max-w-full">Profile</span>
        {location.pathname === "/profile" && (
          <span className="absolute top-0 left-1/2 transform -translate-x-1/2 w-10 h-0.5 bg-gradient-to-r from-teal-400 to-cyan-400 rounded-b"></span>
        )}
      </Link>
